...
def get_stats():
    ...
    # Pomodoro stats
    total_sessions = conn.execute('SELECT COUNT(*) as count FROM pomodoro_sessions').fetchone()['count']
    completed_sessions = conn.execute('SELECT COUNT(*) as count FROM pomodoro_sessions WHERE completed = 1').fetchone()['count']
    work_sessions = conn.execute('SELECT COUNT(*) as count FROM pomodoro_sessions WHERE session_type = "work" AND completed = 1').fetchone()['count']
    break_sessions = conn.execute('SELECT COUNT(*) as count FROM pomodoro_sessions WHERE session_type = "break" AND completed = 1').fetchone()['count']
    
    # Calculate total focus time (completed work sessions * duration)
    focus_time = conn.execute('SELECT SUM(duration) as total FROM pomodoro_sessions WHERE session_type = "work" AND completed = 1').fetchone()['total'] or 0
...
def complete_pomodoro_session(session_id):
    ...
    conn = get_db_connection()
    session = conn.execute('SELECT * FROM pomodoro_sessions WHERE id = ?', (session_id,)).fetchone()
